...
export function canChangeEmail(lastEmailChange: string | Date | null): boolean {
  if (!lastEmailChange) return true;

  const now = new Date();
  const lastChange = new Date(lastEmailChange);
  const daysSinceLastChange =
    (now.getTime() - lastChange.getTime()) / (1000 * 60 * 60 * 24);

  return daysSinceLastChange >= EMAIL_CHANGE_COOLDOWN_DAYS;
}
